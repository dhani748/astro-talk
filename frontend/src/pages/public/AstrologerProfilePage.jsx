...
const AstrologerProfilePage = () => {
  ...
  const handleStartConsultation = async (type) => {
    ...
    try {
      const { data } = await startConsultation({ astrologerId: id, type })
  ...
  if (!astrologer) return <div className="text-center py-20 text-gray-400">Astrologer not found</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="flex flex-col sm:flex-row items-start gap-6">
              ...
              <div className="flex-1">
                <div className="flex items-start justify-between">
                  <div>
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{astrologer.name}</h1>
                    <p className="text-gray-500 dark:text-gray-400 mt-1">{astrologer.specialization}</p>
                  </div>
                ...
                <div className="flex items-center gap-4 mt-3 flex-wrap">
                  <div className="flex items-center gap-1">
                    ...
                    <span className="font-semibold text-gray-900 dark:text-white">{astrologer.rating?.toFixed(1) || '5.0'}</span>
                    <span className="text-sm text-gray-400">({astrologer.reviewCount || 0} reviews)</span>
                  </div>
                  <div className="flex items-center gap-1 text-sm text-gray-500">
                    <FiDollarSign size={16} className="text-primary" />
                    <span className="font-semibold text-primary">₹{astrologer.pricePerMin || 10}/min</span>
                  </div>
                  <div className="flex items-center gap-1 text-sm text-gray-500">
                ...
                <div className="mt-6">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">{astrologer.bio || 'Experienced astrologer with deep knowledge of Vedic astrology, numerology, and tarot reading.'}</p>
                </div>

                <div className="flex flex-wrap gap-2 mt-4">
                  {astrologer.specializations?.map((s, i) => (
                    <span key={i} className="px-3 py-1 bg-primary/10 text-primary rounded-lg text-xs font-medium">{s}</span>
                  ))}
                </div>

                <div className="flex flex-wrap gap-3 mt-6">
                  <button onClick={() => handleStartConsultation('chat')} className="flex items-center gap-2 px-6 py-2.5 bg-primary text-white rounded-xl font-medium hover:bg-primary-dark transition-colors">
                    <FiMessageCircle size={18} /> Chat Now
                  </button>
                  <button onClick={() => handleStartConsultation('call')} className="flex items-center gap-2 px-6 py-2.5 bg-gold text-gray-900 rounded-xl font-medium hover:bg-gold-dark transition-colors">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-8 pt-8 border-t border-gray-100 dark:border-gray-800">
              {[
                ...
                <div key={i} className="text-center p-4 bg-gray-50 dark:bg-gray-800 rounded-xl">
                  <stat.icon className="mx-auto text-primary mb-2" size={22} />
        ...
        <div className="lg:col-span-1">
          <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 border border-gray-100 dark:border-gray-800 sticky top-24">
            <h3 className="font-semibold text-gray-900 dark:text-white mb-4">Book Consultation</h3>
            <div className="space-y-3">
              <button onClick={() => handleStartConsultation('chat')} className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-primary text-white rounded-xl font-medium hover:bg-primary-dark transition-colors">
                <FiMessageCircle /> Start Chat
              </button>
              <button onClick={() => handleStartConsultation('call')} className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-gold text-gray-900 rounded-xl font-medium hover:bg-gold-dark transition-colors">
                <FiPhone /> Start Call
              </button>
            </div>
            <p className="text-xs text-gray-400 text-center mt-4">You will be charged ₹{astrologer.pricePerMin || 10}/minute</p>
          </div>
        </div>
      </div>
    </div>
